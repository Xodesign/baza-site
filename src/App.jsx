...
const INITIAL_OBJECTS = [
	{
		...
		cost: "55 000",
		fullAddress:
			"РФ, Московская область, Одинцовский г.о., тер. Парк Патриот, дом 3",
		address:
			"РФ, Московская область, Одинцовский г.о., тер. Парк Патриот, дом 3",
...
function App() {
	...
	const handleAddCall = (e) => {
		e.preventDefault();
		if (
			!newCallData.customerName.trim() ||
			!newCallData.phone.trim() ||
			!newCallData.address.trim()
		) {
	...
	const handleSaveCall = (e) => {
		e.preventDefault();
		setCalls(
			calls.map((call) => (call.id === editingCall.id ? editingCall : call)),
		);
	...
	const handleAssignEngineer = (id, engineerName) => {
		setCalls(
			calls.map((call) =>
				call.id === id
					? { ...call, engineer: engineerName, status: "in_progress" }
					: call,
			),
		);
	};
	...
	const handleLogin = async (e) => {
		...
		if (authEmail === "admin@baza.ru" && authPassword === "baza123") {
			...
			setAuthError(
				"Неверный email или пароль. Используйте admin@baza.ru / baza123",
			);
		}
	...
	const handleExportFiles = async () => {
		...
		try {
			let csvContent =
				"ID;Наименование объекта;Заказчик;Подрядчик;Договор;Тип договора;Системы;Адрес;Контакт;Оплата;Стоимость\n";
			objects.forEach((obj) => {
	...
	const handleAddObject = (e) => {
		...
		const newObj = {
			...
			lettersToExtraWork: newFormData.lettersToExtraWork || "—",
			...
			documentsType: newFormData.documentsType || "—",
			tenant: newFormData.tenant || "—",
	...
	const handleSaveEdit = (e) => {
		e.preventDefault();
		setObjects(
			objects.map((obj) => (obj.id === editingObject.id ? editingObject : obj)),
		);
			...
			<div className="login-container">
				<div className="login-card">
					...
					<div className="login-hint">
						<p>
							Демо-доступ: <code>admin@baza.ru</code> / <code>baza123</code>
						</p>
					</div>
				</div>
			</div>
												...
												<tr key={obj.id}>
													...
													<td>
														<span className="badge badge-client">
															{obj.customer || obj.client}
														</span>
													</td>
													<td>
														<span
															className={`badge badge-type badge-${obj.contractType?.toLowerCase()}`}
														>
															{obj.contractType || obj.type}
														</span>
													</td>
													<td className="cell-contract">
														{obj.contractNumber || obj.contract}
													</td>
													...
													<td className="cell-address">
														<span className="address-text">
															<MapPin size={14} />
															{obj.shortAddress ||
																obj.address?.substring(0, 30)}
														</span>
													</td>
													<td className="cell-contact">{obj.contact}</td>
													<td>
														<span
															className={`badge badge-payment badge-${obj.payment?.toLowerCase().replace(/\s/g, "-")}`}
														>
															{obj.payment}
														</span>
													</td>
						...
						<>
							...
							<div className="calls-header">
								...
								<div className="filter-tabs">
									<button
										className={`filter-tab ${callFilter === "all" ? "active" : ""}`}
										onClick={() => setCallFilter("all")}
									>
										Все
									</button>
									<button
										className={`filter-tab ${callFilter === "new" ? "active" : ""}`}
										onClick={() => setCallFilter("new")}
									>
										Новые
									</button>
									<button
										className={`filter-tab ${callFilter === "in_progress" ? "active" : ""}`}
										onClick={() => setCallFilter("in_progress")}
									>
										В работе
									</button>
									<button
										className={`filter-tab ${callFilter === "completed" ? "active" : ""}`}
							...
							<div className="calls-list">
								{filteredCalls.length === 0 ? (
									...
									filteredCalls.map((call) => (
										<div
											key={call.id}
											className={`call-card ${call.priority === "high" ? "priority-high" : ""}`}
										>
											<div
											...
											>
												<div className="call-main-info">
													<div className="call-badges">
														<span
															className={`badge ${getStatusBadgeClass(call.status)}`}
														>
															{getStatusLabel(call.status)}
														</span>
														<span
															className={`badge ${getPriorityBadgeClass(call.priority)}`}
														>
												...
												<div className="call-details">
													<div className="call-details-grid">
														<div className="detail-section">
															...
															<p>
																<strong>{call.contactPerson}</strong>
															</p>
														...
														<div className="detail-section">
															...
															<p>{call.address}</p>
															{call.extraData?.objectName && (
																<p className="detail-sub">
																	Объект: {call.extraData.objectName}
																</p>
															)}
														</div>
														<div className="detail-section">
															<h4>
																<FileTextIcon size={16} /> Причина обращения
															</h4>
															<p>{call.reason}</p>
															<p className="detail-sub">
																Система: {call.system}
														...
														<div className="detail-section">
															<h4>
																<Settings size={16} /> Дополнительные данные
															</h4>
															{call.extraData?.contractNumber && (
																<p>Договор: {call.extraData.contractNumber}</p>
															)}
															{call.extraData?.equipment && (
																<p>Оборудование: {call.extraData.equipment}</p>
															)}
															{call.extraData?.note && (
														...
														{call.status === "new" && (
															<div className="action-group">
																...
																>
																	...
																	<option value="Петров В.И.">
																		Петров В.И.
																	</option>
																	<option value="Сидоров А.П.">
																		Сидоров А.П.
																	</option>
																	<option value="Михайлов А.В.">
																		Михайлов А.В.
																	</option>
																	<option value="Козлов Н.С.">
																		Козлов Н.С.
																	</option>
																</select>
															</div>
							...
							<div className="add-call-section">
								<h3>
									<Plus size={20} /> Создать новый вызов
								</h3>
										...
										<div className="form-row">
											<div className="form-group">
												...
												<input
													...
													placeholder="ООО Компания или ИП Фамилия"
												/>
											</div>
											<div className="form-group">
												...
												<input
													type="text"
													value={newCallData.contactPerson}
													onChange={(e) =>
														setNewCallData({
															...newCallData,
										...
										<div className="form-row">
											<div className="form-group form-group-full">
												...
												<input
													...
													placeholder="Опишите причину вызова"
												/>
											</div>
										</div>
										<div className="form-row">
									...
									<div className="form-section">
										...
										<div className="form-row">
											<div className="form-group">
												...
												<input
													type="text"
													value={newCallData.extraData.objectName}
													onChange={(e) =>
														setNewCallData({
															...newCallData,
															extraData: {
																...newCallData.extraData,
																objectName: e.target.value,
															},
														})
													}
													placeholder="Бизнес-центр Пример"
												/>
											</div>
											<div className="form-group">
												<label>Номер договора</label>
												<input
													type="text"
													value={newCallData.extraData.contractNumber}
													onChange={(e) =>
														setNewCallData({
															...newCallData,
															extraData: {
										...
										<div className="form-row">
											<div className="form-group">
												...
												<input
													type="text"
													value={newCallData.extraData.equipment}
													onChange={(e) =>
														setNewCallData({
															...newCallData,
															extraData: {
													...
													placeholder="Название и модель оборудования"
												/>
											</div>
											<div className="form-group">
												<label>Заметка</label>
												<input
													type="text"
													value={newCallData.extraData.note}
													onChange={(e) =>
														setNewCallData({
															...newCallData,
															extraData: {
						...
						<div className="placeholder-section">
							<div className="placeholder-icon">
								{MENU_ITEMS.find((m) => m.id === activeTab)?.icon &&
									(() => {
										const IconComponent = MENU_ITEMS.find(
											(m) => m.id === activeTab,
										).icon;
							...
							<div className="form-grid">
								...
								<div className="form-group">
									...
									<input
										...
										onChange={(e) =>
											handleEditChange("customer", e.target.value)
										}
									/>
								</div>
								<div className="form-group">
									...
									<select
										value={
											editingObject.contractType || editingObject.type || "ТО"
										}
										onChange={(e) =>
								...
								<div className="form-group">
									...
									<input
										type="text"
										value={
											editingObject.contractNumber ||
											editingObject.contract ||
											""
										}
										onChange={(e) =>
								...
								<div className="form-group">
									...
									<input
										...
										onChange={(e) =>
											handleEditChange("systems", e.target.value)
										}
									/>
								</div>
								<div className="form-group">
									...
									<input
										type="text"
										value={
											editingObject.fullAddress || editingObject.address || ""
										}
										onChange={(e) =>
											handleEditChange("fullAddress", e.target.value)
										}
									/>
								</div>
								<div className="form-group">
								...
								<div className="form-group">
									...
									<input
										...
										onChange={(e) =>
											handleEditChange("payment", e.target.value)
										}
									/>
								</div>
								<div className="form-group">
								...
								<div className="form-group">
									...
									<select
										...
										onChange={(e) =>
											handleEditChange("toolsOnSite", e.target.value)
										}
									>
										<option value="нет">Нет</option>
										<option value="да">Да</option>
								...
								<div className="form-row">
									<div className="form-group">
										...
										<input
											...
											onChange={(e) =>
												handleEditCallChange("customerName", e.target.value)
											}
										/>
									</div>
									<div className="form-group">
										...
										<input
											type="text"
											value={editingCall.contactPerson || ""}
											onChange={(e) =>
												handleEditCallChange("contactPerson", e.target.value)
											}
										/>
									</div>
									<div className="form-group">
										...
										<input
											type="tel"
											value={editingCall.phone || ""}
											onChange={(e) =>
												handleEditCallChange("phone", e.target.value)
											}
										/>
									</div>
								</div>
							...
							<div className="form-section">
								...
								<div className="form-row">
									<div className="form-group form-group-full">
										...
										<input
											type="text"
											value={editingCall.address || ""}
											onChange={(e) =>
												handleEditCallChange("address", e.target.value)
											}
										/>
									</div>
								</div>
								<div className="form-row">
									<div className="form-group form-group-full">
										...
										<input
											type="text"
											value={editingCall.reason || ""}
											onChange={(e) =>
												handleEditCallChange("reason", e.target.value)
											}
										/>
									</div>
								</div>
								<div className="form-row">
									<div className="form-group">
										<label>Система</label>
										<select
											value={editingCall.system || "АПС"}
											onChange={(e) =>
												handleEditCallChange("system", e.target.value)
											}
										>
											<option value="АПС">АПС</option>
											<option value="СОУЭ">СОУЭ</option>
									...
									<div className="form-group">
										<label>Приоритет</label>
										<select
											value={editingCall.priority || "normal"}
											onChange={(e) =>
												handleEditCallChange("priority", e.target.value)
											}
										>
											<option value="normal">Обычный</option>
											<option value="high">Высокий</option>
									...
									<div className="form-group">
										...
										<select
											...
											onChange={(e) =>
												handleEditCallChange("status", e.target.value)
											}
										>
								...
								<div className="form-row">
									<div className="form-group form-group-full">
										...
										<textarea
											...
											onChange={(e) =>
												handleEditCallChange("description", e.target.value)
											}
											rows={2}
										/>
									</div>
							...
							<div className="form-section">
								...
								<div className="form-row">
									<div className="form-group">
										...
										<input
											type="text"
											value={editingCall.extraData?.objectName || ""}
											onChange={(e) =>
												handleEditCallExtraChange("objectName", e.target.value)
											}
										/>
									</div>
									<div className="form-group">
										...
										<input
											type="text"
											value={editingCall.extraData?.contractNumber || ""}
											onChange={(e) =>
												handleEditCallExtraChange(
													"contractNumber",
													e.target.value,
												)
											}
								...
								<div className="form-row">
									<div className="form-group">
										...
										<input
											type="text"
											value={editingCall.extraData?.equipment || ""}
											onChange={(e) =>
												handleEditCallExtraChange("equipment", e.target.value)
											}
										/>
									</div>
									<div className="form-group">
										...
										<input
											type="text"
											value={editingCall.extraData?.note || ""}
											onChange={(e) =>
												handleEditCallExtraChange("note", e.target.value)
											}
										/>
									</div>
								</div>
